...
#ifndef DDMD_HOOMD_CONFIG_IO_CPP
#define DDMD_HOOMD_CONFIG_IO_CPP
...
namespace DdMd
{
   ...
   * Constructor.
   */
   HoomdConfigIo::HoomdConfigIo(bool hasMolecules)
    : ConfigIo(),
      hasMolecules_(hasMolecules)
   {  setClassName("HoomdConfigIo"); }

   /*
   * Constructor.
   */
   HoomdConfigIo::HoomdConfigIo(Simulation& simulation, bool hasMolecules)
    : ConfigIo(simulation),
      hasMolecules_(hasMolecules)
   {  setClassName("HoomdConfigIo"); }
   ...
   */
   template <int N>
   int HoomdConfigIo::readGroups(std::ifstream& file, 
   ...
   */
   void HoomdConfigIo::readConfig(std::ifstream& file, MaskPolicy maskPolicy)
   {
      // Precondition
   ...
   */
   template <int N>
   int HoomdConfigIo::writeGroups(std::ofstream& file, 
                  const char* sectionLabel,
                  const char* nGroupLabel,
   ...
   */
   void HoomdConfigIo::writeConfig(std::ofstream& file)
   {
      // Precondition
      if (domain().isMaster() && !file.is_open()) {  
            UTIL_THROW("Error: File is not open on master"); 
