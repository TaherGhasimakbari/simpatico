...
namespace McMd
{
   ...
   ClustersDynamics::ClustersDynamics(System& system) 
    : SystemAnalyzer<System>(system),
      ...
      clusters1_(system),
      clusters2_(system),
      oldClustersPtr_(&clusters1_),
      newClustersPtr_(&clusters2_),
      speciesId_(),
      speciesPtr_(),
      coreId_(),
      histMin_(0),
   ...
   void ClustersDynamics::readParameters(std::istream& in) 
   {
      ...
      clusters1_.speciesId_ = speciesId_;
      clusters2_.speciesId_ = speciesId_;

      read<int>(in, "coreId", coreId_);
      if (coreId_ < 0) {
         UTIL_THROW("Negative coreId");
      }
      clusters1_.coreId_ = coreId_;
      clusters2_.coreId_ = coreId_;
      
      read<double>(in, "cutoff", cutoff_);
      if (cutoff_ < 0) {
         UTIL_THROW("Negative cutoff");
      }
      clusters1_.cutoff_ = cutoff_;
      clusters2_.cutoff_ = cutoff_;

      read<int>(in,"histMin", histMin_);
      read<int>(in,"histMax", histMax_);
      clusters1_.hist_.setParam(histMin_, histMax_);
      clusters2_.hist_.setParam(histMin_, histMax_);
   ...
   */
   ...
   { 
      if (isAtInterval(iStep)) {
         ...
         ClustersFinder *temp = oldClustersPtr_;
         oldClustersPtr_ = newClustersPtr_;
         newClustersPtr_ = temp;
         ...
         for (int i = 0; i < newClustersPtr_->clusterLengths_.size(); i++) {
             for (int j=0; j < oldClustersPtr_->clusterLengths_.size(); j++) {
                 if (clustersCorrelations_[i][j] > criterion_ * oldClustersPtr_->clusterLengths_[i]) id_ = j;
             }
             usedIds_[id_] = true;
         ...
         for (int i = 0; i < newClustersPtr_->clusterLengths_.size(); i++) {
             for (int j=0; j < oldClustersPtr_->clusterLengths_.size(); j++) {
                 outputFile_<<"("<<newClustersPtr_->clusterLengths_[i]<<") =";
                 if (clustersCorrelations_[i][j] != 0){
                    outputFile_<<"["<<oldClustersPtr_->clusterLengths_[j]<<"]("<<clustersCorrelations_[i][j]<<") ";
                 }
             }
             outputFile_<<"\n";
         }
